---
title: LLM Agent Integration Guide
description: A copy-pasteable prompt for AI coding assistants integrating this SDK into a project
---

Working with Claude, Cursor, Copilot, or another AI coding assistant? Paste
the guide below into your prompt (or save it as a file the agent can read)
to have it wire up ABA PayWay in your project correctly on the first try.

<Callout title="Why this exists" type="info">
An earlier version of this guide told agents to call `createDynamicKHQR()`
and `checkTransaction()` — neither method exists on `ABAPayWay`. Any agent
that followed it literally generated code that failed to compile. This
version only references real methods and field names.
</Callout>
...
- [Configuration](/docs/configuration) — what each credential-sheet value maps to
- [Creating Purchases](/docs/create-purchase) — the full `PurchaseResponse` shape
- [Checking Status](/docs/check-status) — the full `StatusResponse` shape and error codes
